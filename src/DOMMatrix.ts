...
export class DOMMatrixReadOnly {
	 ...
	 */
	constructor(init: string | ArrayLike<number> = default2DValues) {
	...
	get isIdentity() {
		for (var i = 0, len = this[elements].length; i < len; i += 1) {
			if (this[elements][i] !== identityValues[i]) return false
		}
	...
	static fromMatrix(other: DOMMatrixReadOnly) {
		const mat = other[elements as unknown as keyof typeof other] as unknown as number[]
		return new this(mat)
	}
...
export class DOMMatrix extends DOMMatrixReadOnly {
	constructor(arg?: string | ArrayLike<number>) {
		if (arg === undefined) {
			...
			super(arg)
		} else throw new Error('Invalid argument to DOMMatrix constructor.')
	}
	...
	scaleSelf(scaleX = 1, scaleY = scaleX, scaleZ = 1, originX = 0, originY = 0, originZ = 0) {
		...
		if (scaleZ !== 1 || originZ !== 0) this[is2D] = false
...
function getInit(mat: DOMMatrixReadOnly) {
	return mat.is2D ? [mat.m11, mat.m12, mat.m21, mat.m22, mat.m41, mat.m42] : mat.toFloat64Array()
}

function isNumberSequence(obj: any): obj is ArrayLike<number> {
	if (obj && typeof obj.length === 'number' && typeof obj[0] === 'number') return true
